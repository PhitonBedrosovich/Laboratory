  ...
  <div :class="$style.root">
    ...
    <div>
      <div :class="$style.item">
        ...
        <input type="file" id="img_path" accept="image/*" @change="onFileChange">
      </div>
      <img v-if="form.file" :src="form.file" alt="Image" style="max-width: 200px;"/>
    </div>
    <div :class="$style.item">
      <div :class="$style.label">
    ...
    <div :class="$style.item">
      <div :class="$style.label">
        <label for="id_dishes">Меню</label>
      </div>
...
import { selectItemById, fetchItems } from '@/store/foods/selectors';
import { selectItems as selectMenus, fetchMenus } from '@/store/menus/selectors';
...
export default {
  ...
  setup(props, context) {
    ...
    watchEffect(() => {
      const menu = selectItemById( store,  props.id );
    ...
    return {
      menuList,
      form,
      isValidForm: computed(() =>  !!(form.name && form.description && form.weight  && form.id_dishes)),
      onClick: () => {

  ...
  <div :class="$style.root">
    ...
    >
      <template v-slot:control="{ item }">
        ...
        <Btn @click="onClickShow(item.id)" theme="info">Список блюд</Btn>
      </template>
    </Table>
...
import { selectItems, removeItem, fetchMenus  } from '@/store/menus/selectors'
...
export default {
  ...
  setup() {
    ...
    return {
      ...
      onClickShow: (id) => {
        router.push({name: 'FoodsMenu', query: {dishID: id}})
      },
    }
  }
}

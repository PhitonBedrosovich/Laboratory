...
abstract class TableModule
{
    ...
    public function getRecordsByFilter($id_dishes, $fields = array())
    {
        $sql = "SELECT * FROM " . $this->getTableName() . " WHERE id_dishes=:id_dishes";
        foreach ($fields as $key => $field) {
            $sql .= " AND " . $key . " = :" . $key;
        }
        $query = Singleton::prepare($sql);
        $query->execute(array_merge(array(':id_dishes' => $id_dishes), $fields));
        $result = array();
        while ($slice = $query->fetch()) {
            $result[] = $slice;
        }
